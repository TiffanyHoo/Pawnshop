...
export default class ManagePawn extends Component {
        ...
        render: (_, record) =>
          this.state.dataSource.length >= 1 ? (
            <div>
              <Popconfirm
                title="确认通知吗？"
                onConfirm={() => this.handleNotification(record)}
              >
                <a>通知</a>
              </Popconfirm>
              &nbsp;&nbsp;&nbsp;&nbsp;
  ...
  exportExcel = (filename) => {
    ...
    linkElement.href =
      'http://localhost:8080/myDemo/system/temp/' + filename + '.xls';
  ...
  exportPdf = (filename) => {
    ...
    linkElement.style.display = 'none';
    linkElement.href =
      'http://localhost:8080/myDemo/system/temp/' + filename + '.pdf';
      ...
      notification.open({
        ...
        description: (
          <div style={{ whiteSpace: 'pre-wrap' }}>
            已成功打印，票据保存在C:\apache-tomcat-8.0.32\webapps\myDemo\system\temp
          </div>
        ),
  ...
  InsertInfo = () => {
    ...
    if (PubType === '到期提醒') {
      Content =
        ...
        GenderText +
        '，您在' +
        store.getState().PSName +
        '的当单(' +
        PTID +
        ')已到期，请尽快赎回。如有特殊情况，请联系' +
        store.getState().PSPhone +
        ',感谢您的配合~';
  ...
  pubNotification = () => {
    ...
    axios({
      ...
      notification.open({
        message: '消息',
        description: (
          <div style={{ whiteSpace: 'pre-wrap' }}>已成功发送通知给当户~</div>
        ),
        icon: <SmileOutlined style={{ color: 'orange' }} />,
        duration: 2,
      });
    ...
    return (
      <div>
        ...
        <Drawer
          ...
          extra={
            <Space>
              <Button onClick={this.printTicket} type="primary">
                打印
              </Button>
            </Space>
          }
        >
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item name="Notes" label="当单标注">
                  <Input
