...
export default class CreatePawn extends Component {
    ...
    this.columns = [
      {
        ...
        render: (_, record) =>
          this.state.dataSource.length >= 1 ? (
            <Popconfirm
              title="确认转换为当单吗？"
              onConfirm={() => this.changeApply(record)}
            >
              <a>&nbsp;&nbsp;转换</a>
            </Popconfirm>
          ) : null,
    ...
    axios
      ...
      .then((response) => {
        if (response.data.length === 0) {
          ...
          const { UserName, Gender, Phone, Address, Email, Wechat } =
            response.data[0];
          this.setState({
            UserName,
            Gender,
  ...
  showDrawer2 = () => {
    this.setPTID();
    const { UserID, UserName, Address, Phone, Email, Gender, Wechat, TID } =
      this.state;
    this.getThings(TID);
    this.setState({
      visible: true,
  ...
  onSubmit = async () => {
    const { UserID, UserName, Gender, Phone, Email, Wechat, Address, PTID } =
      this.state;
    const BirthDate =
      ...
      notification.open({
        ...
        description: (
          <div style={{ whiteSpace: 'pre-wrap' }}>
            已成功建当，当单编号为{PTID},可于当单信息管理模块中查看~
          </div>
        ),
  ...
  saveItem = () => {
    ...
    const pawnitem = {
      ...
      Documents,
      Quantity,
      AssessPrice: AssessPrice ? AssessPrice : 0,
      Rate: Rate ? Rate : 0,
    ...
    axios
      ...
      .then((response) => {
        if (response.data.length === 0) {
          ...
          const dataSource1 = [
            { ...response.data[0], PIID: UIID, key: UIID, fileList },
          ];
          that.setState({
            dataSource1,
            isSearch: false,
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <Row gutter={16} className="myRow">
              <Col span={24}>
                <Form.Item name="Notes" label="当单备注：">
                  <Input
            ...
            >
              <Tooltip title="查找当品">
                <Button
                  ...
                  icon={<SearchOutlined />}
                  onClick={() =>
                    this.setState({ isSearch: true, searchID: '' })
                  }
        ...
        <Modal
          ...
          onOk={this.searchItem}
          onCancel={() => this.setState({ isSearch: false })}
        >
          <Input
