...
const getRandomuserParams = (params) => ({
  ...
  ApplyType:
    params.ApplyType == undefined || params.ApplyType == null
      ? '1,2,3,4'
      : params.ApplyType.join(','),
  state:
...
export default class ManageApply extends Component {
        ...
        render: (_, record) =>
          this.state.dataSource.length >= 1 ? (
            <div>
              ...
              <Popconfirm
                title="确认删除吗？"
                onConfirm={() => this.handleDelete(record.ApplyID)}
              >
                <a>删除</a>
              </Popconfirm>
            </div>
  ...
  getData = async (params = {}) => {
    ...
    ApplyType =
      ApplyType == undefined || ApplyType == null
        ? '1,2,3,4'
        : ApplyType.join(',');
    ...
    fetch(
      `http://localhost:3000/getApply?${Qs.stringify(
        getRandomuserParams(params, ApplyType, state)
      )}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  //报价单
  showModal = (record) => {
    const { ApplyID, ApplyType, Amount, ANotes, state } =
      record.ApplyID !== undefined ? record : this.state;
    if (state == 0) {
      notification.open({
        message: '消息',
        description: (
  ...
  //保存报价
  handleSave = async () => {
    const { ApplyID, Amount, ANotes, pagination, visible1, visible2 } =
      this.state;
    let data = {
      ApplyID,
      Amount,
    ...
    notification.open({
      message: '消息',
      description: <div style={{ whiteSpace: 'pre-wrap' }}>已成功报价</div>,
      ...
      <div>
              ...
              return {
                onDoubleClick: (event) => {
                  ...
                  this.setState({
                    ...
                    Total,
                    ENotes,
                    visible1: ApplyType == '1' ? true : false,
                    visible2: ApplyType != '1' ? true : false,
                  });
                  setTimeout(() => {
                    if (ApplyType == '1') {
                      this.formRef.current.setFieldsValue({
        ...
        >
          ...
          >
            <Row gutter={16} className="myRow">
              <Col span={6}>
                <Form.Item name="ApplyID" label="申请单编号：">
                  <Input readOnly />
                </Form.Item>
              </Col>
              <Col span={6}>
              ...
              <Col span={6}>
                <Form.Item name="ApplyDate" label="申请时间：">
                  <Input readOnly />
                </Form.Item>
              </Col>
              <Col span={6}>
                <Form.Item name="Plandate" label="计划到行时间：">
                  <Input readOnly />
                </Form.Item>
              </Col>
            </Row>
            ...
            <Row gutter={16} className="myRow">
              <Col span={8}>
                <Form.Item name="Phone" label="联 系 电 话：">
                  <Input readOnly />
                </Form.Item>
              </Col>
              <Col span={8}>
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item name="detail" label="当品明细">
                  <p style={{ margin: 0, minHeight: 0 }}>
                    当品份数 : {this.state.dataSource1.length}
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;期望到手价 :{' '}
                    {this.state.ExpectAmount}
                  </p>
                </Form.Item>
              </Col>
            </Row>
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item name="Amount" label="预估价">
                  <p
                    style={{
                      margin: 0,
                      minHeight: 0,
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item name="ANotes" label="备注：">
                  <Input readOnly />
                </Form.Item>
              </Col>
            </Row>
        ...
        >
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item name="detail" label="物品明细">
                  <p style={{ margin: 0, minHeight: 0 }}>
                    物品份数 : {this.state.dataSource1.length}
                  </p>
                </Form.Item>
              </Col>
            </Row>
        ...
        >
          ...
          >
            ...
            <Form.Item name="ANotes" label="报 价 备 注">
              <Input.TextArea
                rows={2}
                style={{ resize: 'none' }}
                onChange={(e) => {
                  this.setState({ ANotes: e.target.value });
                }}
                placeholder="请输入报价备注"
              />
            </Form.Item>
