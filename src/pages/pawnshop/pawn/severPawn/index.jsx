...
export default class SeverPawn extends Component {
    ...
    this.columns = [
      {
        ...
        render: (_, record) =>
          this.state.dataSource.length >= 1 ? (
            <div>
              <Popconfirm title="确认绝当吗？" onConfirm={() => this.showModal(record)}>
                <a>绝当</a>
              </Popconfirm>
            </div>
          ) : null
  ...
  onSubmit2 = async () => {
    ...
    for (const item of dataSource1) {
      ...
      let data = {
        step: 2,
        PTID:newPTID,PIID,AssessPrice,Rate,Amount
      }
  ...
  onSubmit3 = async () => {
    const {newPTID,Interest,StoreFare,OverdueFare,FreightFare,AuthenticateFare,AssessFare,NotaryFare,InsuranceFare,OtherFare,ENotes } = this.state
    let data = {
      step: 3,
      PTID: newPTID,PSstaffID:store.getState().PSstaffID,
      Interest,StoreFare,OverdueFare,FreightFare,AuthenticateFare,AssessFare,NotaryFare,InsuranceFare,OtherFare,ENotes
    }
  ...
  showModal = async (record) => {
    ...
    if(record.UserID!=undefined){
      ...
      this.setState({
        PTID,EndDate,UserID,UserName,TotalPrice,Quantity,
        visible_modal: true,newEndDate:EndDate
      });
      setTimeout(() => {
        this.formRef2.current.setFieldsValue({
          newPTID:newPTID,PTID,PawnDate:[moment(nowDate),moment(nowDate)],CusInfo:UserID+"  "+UserName,TotalPrice,Quantity,Interest: 0,StoreFare: 0,OverdueFare: 0,FreightFare: 0,AuthenticateFare: 0,AssessFare: 0,NotaryFare: 0,InsuranceFare: 0,OtherFare: 0,Notes: '',ENotes: ''
        })
      }, 200);
    }else{
      const { newPTID,nowDate,PTID,EndDate,UserID,UserName,TotalPrice,Quantity } = this.state;
      this.setState({
        PTID,EndDate,UserID,UserName,TotalPrice,Quantity,
        visible_modal: true,newEndDate:EndDate
      });
  ...
  render() {
    const { nowDate,newPTID,TotalExpense,ImageVisible,expandedRowKeys,dataSource,dataSource1,PIID,CID,UserID,UserName,Gender,Address,Phone,Email,Wechat,PSID,Specification,Documents,photopath,state,PriceOnSale,canDistribute,title,SpeDetail,DocDetail,PTID,Quantity,TotalPrice,PSstaffIDA,Notes,StartDate,EndDate,newEndDate} = this.state;    
    const components = {
      body: {
        row: EditableRow,
        cell: EditableCell,
        ...
        <div className="site-layout-background" style={{ padding: 10 }}>
          <Table
            ...
            onRow={record => {
              return {
                onDoubleClick: event => {
                  const { PTID,UserID,UserName,Gender,Address,Phone,Email,Wechat,StartDate,EndDate,PSstaffIDA,Notes,Quantity,TotalPrice,Interest,StoreFare,OverdueFare,FreightFare,AuthenticateFare,AssessFare,NotaryFare,InsuranceFare,OtherFare,Total,ENotes } = record
                  this.getDetail(PTID)
                  this.setState({
                    PTID,UserID,UserName,StartDate,EndDate,PSstaffIDA,Notes,Quantity,TotalPrice,Interest,StoreFare,OverdueFare,FreightFare,AuthenticateFare,AssessFare,NotaryFare,InsuranceFare,OtherFare,Total,ENotes,
                    visible: true
                  });
                  setTimeout(() => {
                    this.formRef.current.setFieldsValue({
                      PTID,UserID,UserName,Gender:Gender==='0'?'男':'女',Address,Phone,Email,Wechat,Interest,StoreFare,OverdueFare,FreightFare,AuthenticateFare,AssessFare,NotaryFare,InsuranceFare,OtherFare,Total,ENotes
                    })
                  }, 200);
                },
              };
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item
                  ...
                  label="当单详情"
                >
                  <p style={{padding:0,margin:0}}>当物数量 : {this.state.Quantity}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;总当价 : {this.state.TotalPrice}</p>
                </Form.Item>
              </Col>
            </Row>        
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={12}>
                ...
                >
                  <RangePicker style={{width:'100%'}} 
                  disabledDate={(current)=>{return current <= moment(EndDate).subtract(1, "days")}} 
                  ...
                  <Col span={24}>
                    ...
                    >
                      <Space size={20} align='start' style={{display:'flex'}}>
                        ...
                        <Space size={5} direction="vertical">
                            <p style={{marginBottom:'0'}}>当品编号 : {PIID}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当品名称 : {title}</p>
                            <div>
                              估价 : <InputNumber defaultValue={AssessPrice} style={{width:'100px'}} height={22} bordered={false}
                              formatter={value => `￥${value}`}
                              onChange={(e)=>this.handlePrice(e,PIID,'AssessPrice')}/>
                              折价率 : <InputNumber defaultValue={Rate} style={{width:'60px'}} height={22} bordered={false}
                              min={0} max={100}
                              formatter={value => `${value} %`}
                              onChange={(e)=>this.handlePrice(e,PIID,'Rate')}/>
                              当价 : <InputNumber readOnly value={dataSource1[index].Amount} style={{width:'100px'}} height={22} bordered={false}
                              formatter={value => `￥${value}`}/>
                            </div>
                            <p style={{marginBottom:'0'}}>物品详情 : {Specification?Specification:'无'}</p>
                            <p style={{marginBottom:'0'}}>包含附件 : {Documents?Documents:'无'}</p>
                        </Space>
                      </Space>
                    </Form.Item>
                  </Col>
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item
                  ...
                  label="当单标注"
                >
                  <Input.TextArea rows={2} onChange={(e)=>this.setState({Notes:e.target.value})} placeholder="请输入当单标注内容" />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={16}>
              <Col span={24}>
                <p>费用单详情&nbsp;&nbsp;<span style={{margin:'0 auto', color:'orange'}}>总费用 : {TotalExpense}</span></p>
              </Col>
            </Row>
            <Row gutter={16}>
              <Col span={8}>
