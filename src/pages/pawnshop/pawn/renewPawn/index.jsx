...
import { Breadcrumb, Table, Input, InputNumber, Button, Popconfirm, Form, Drawer, Col, Row, Select, DatePicker, Space, Tooltip, notification, Modal, Tag, Image } from 'antd'
    ...
    this.columns = [
      {
        ...
        render: (_, record) =>
          this.state.dataSource.length >= 1 ? (
            <div>
              <Popconfirm title="确认续当吗？" onConfirm={() => this.showModal(record)}>
                <a>续当</a>
              </Popconfirm>
            </div>
          ) : null
    ...
    if(record.UserID!=undefined){
      ...
      setTimeout(() => {
        this.formRef2.current.setFieldsValue({
          newPTID:newPTID,PTID,PawnDate:[moment(nowDate),moment(EndDate)],CusInfo:UserID+"  "+UserName,TotalPrice,Quantity,Interest: 0,StoreFare: 0,OverdueFare: 0,FreightFare: 0,AuthenticateFare: 0,AssessFare: 0,NotaryFare: 0,InsuranceFare: 0,OtherFare: 0,Notes: '',ENotes: ''
        })
      }, 200);
    }else{
            ...
            onRow={record => {
              return {
                onDoubleClick: event => {
                  ...
                  this.getDetail(PTID)
                  this.setState({
                    PTID,UserID,UserName,StartDate,EndDate,PSstaffIDA,Notes,Quantity,TotalPrice,
                    visible: true
                  });
                  setTimeout(() => {
                    this.formRef.current.setFieldsValue({
                      PTID,UserID,UserName,PawnDate:[moment(StartDate), moment(EndDate)],PSstaffIDA,Notes,Quantity,TotalPrice,UserID,UserName,Gender:Gender==='0'?'男':'女',Address,Phone,Email,Wechat,Interest,StoreFare,OverdueFare,FreightFare,AuthenticateFare,AssessFare,NotaryFare,InsuranceFare,OtherFare,Total,ENotes
                    })
                  }, 200);
                },
              };
