...
import { AuditOutlined, EyeOutlined, UserOutlined } from '@ant-design/icons';
    ...
    <Layout>
      <Header className="header">
        ...
        <Button
          ...
          onClick={() => {
            store.dispatch(createCommerceAction({}));
        ...
        <p style={{ float: 'right', color: 'white', marginRight: 50 }}>
          操作人员： {JSON.parse(sessionStorage.getItem('userinfo')).ComMemName}
        </p>
      </Header>
      <Layout>
        <Sider collapsible collapsed={state.collapsed} onCollapse={onCollapse}>
          <Menu theme="dark" defaultSelectedKeys={['1']} mode="inline">
            <SubMenu key="sub1" icon={<AuditOutlined />} title="资质管理">
              <Menu.Item key="1">
                <MyNavLink replace to={`qualification/pawnshopinfo`}>
                  当行信息管理
                </MyNavLink>
              </Menu.Item>
              <Menu.Item key="2">
                <MyNavLink replace to={`qualification/registrationaudit`}>
                  注册审核
                </MyNavLink>
              </Menu.Item>
              <Menu.Item key="3">
                <MyNavLink replace to={`qualification/infochangeaudit`}>
                  信息变更审核
                </MyNavLink>
              </Menu.Item>
            </SubMenu>
            <SubMenu key="sub2" icon={<EyeOutlined />} title="典当监管">
              <Menu.Item key="4">
                <MyNavLink replace to={`supervise/pawnitems`}>
                  当物合规管理
                </MyNavLink>
              </Menu.Item>
              <Menu.Item key="5">
                <MyNavLink replace to={`supervise/pawner`}>
                  当户信息管理
                </MyNavLink>
              </Menu.Item>
            </SubMenu>
            <SubMenu key="sub3" icon={<UserOutlined />} title="信息管理">
              <Menu.Item key="6">
                <MyNavLink replace to={`personalInfo`}>
                  个人信息管理
                </MyNavLink>
              </Menu.Item>
            </SubMenu>
        ...
        <Layout className="site-layout">
          <Content style={{ margin: '10px 16px' }}>
            ...
            >
              <Routes>
                <Route
                  path="qualification/pawnshopinfo"
                  element={<PawnshopInfo />}
                />
                <Route
                  path="qualification/registrationaudit"
                  element={<RegistrationAudit />}
                />
                <Route
                  path="qualification/infochangeaudit"
                  element={<InfoChangeAudit />}
                />
